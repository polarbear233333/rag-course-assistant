...
KEYWORDS = {
    ...
    "graph_algorithms": ["dijkstra", "bellman", "mst", "最短路", "最小生成树", "图论"],
...
def profile_summary() -> Dict:
    ...
    distribution = [{"topic": topic, "count": count, "ratio": round(count / total, 3)} for topic, count in topics.items()]
    weak = [item["topic"] for item in distribution[-3:]] if distribution else []
    strong = [item["topic"] for item in distribution[:3]] if distribution else []
